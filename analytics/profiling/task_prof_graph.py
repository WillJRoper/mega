...
# Color dict 
cols = {"Writing": "red", "Collecting": "gold", "Linking": "yellowgreen", "Reading": "darkorchid",
        "Assigning": "darkgreen", "Worker Idle": 'lightskyblue', "Master Idle": 'violet'}
...
for rank in range(nranks):
    ...
    with open(inputs["profilingPath"] + "Graph_" + str(rank) + '_' + snap + '.pck', 'rb') as pfile:
    ...
    for task_type in prof_dict:
        ...
        # for task_start, task_end in zip(starts, ends):
            # print(rank, task_type, task_start, task_end)
            # ax.plot((task_start, task_end), ( rank, rank), color=cols[task_type], label=task_type)
            # ax.fill_between((task_start, task_end), y1=rank - 0.5, y2=rank + 0.5,
            #                 facecolor=cols[task_type], edgecolor=cols[task_type], label=task_type, alpha=0.9)
...
fig.savefig(inputs["analyticPlotPath"] + "graph_task_time_series_" + str(snap) + ".png", bbox_inches="tight", dpi=300)
...
fig.savefig(inputs["analyticPlotPath"] + "graph_task_pie_chart_" + str(snap) + ".png", bbox_inches="tight")
...
fig.savefig(inputs["analyticPlotPath"] + "graph_task_bar_chart_" + str(snap) + ".png", bbox_inches="tight")
